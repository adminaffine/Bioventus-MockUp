...
import type { CFOClosure as CFOClosureData } from "../../services/api";
import { useCFOWorkflow } from "../../context/CFOWorkflowContext";
import {
  buildCfoKpiImpactNoteLines,
  closureKpiRows,
  formatCFOKpiValue,
  CFO_KPI_LABELS,
} from "../../utils/cfoClosureFormat";

const money = (n: number) =>
  n.toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 });

export default function CFOClosure() {
  const { alertId } = useParams<{ alertId: string }>();
  const navigate = useNavigate();
  const { approveAlert, refreshDashboard } = useCFOWorkflow();
  const [data, setData] = useState<CFOClosureData | null>(null);
  ...
  const kpiRows = closureKpiRows(data.kpi_impact);
  const kpiImpactNoteLines = buildCfoKpiImpactNoteLines(data.kpi_impact);
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/20 p-8 text-center">
        ...
        <h1 className="mt-4 text-2xl font-bold text-emerald-900 dark:text-emerald-100">Financial Exposure Resolved ✓</h1>
        <p className="mt-2 text-emerald-800 dark:text-emerald-200">
          Exposure recovered: {money(rc.exposure_recovered)}
        </p>
        <p className="mt-1 text-sm text-emerald-700 dark:text-emerald-300">CFO dashboard KPIs have been updated.</p>
      </section>
        ...
        <table className="mt-4 min-w-full text-sm">
          ...
          <tbody>
            {kpiRows.map(([key, vals]) => {
              const beforeStr = formatCFOKpiValue(key, vals.before);
              const afterStr = formatCFOKpiValue(key, vals.after);
              const changed = beforeStr !== afterStr;
              return (
                <tr key={key} className="border-b border-slate-100 dark:border-slate-700/60">
                  <td className="py-2 pr-4 font-medium">{CFO_KPI_LABELS[key] ?? key}</td>
