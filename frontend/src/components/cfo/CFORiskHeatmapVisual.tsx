...
import type { CFOHeatmapBarRow } from "../../utils/cfoDashboard";
import { cfoHeatmapIssueTypeShortName, heatmapSeverityHex } from "../../utils/cfoDashboard";
...
type ChartRow = CFOHeatmapBarRow & {
  chartLabel: string;
  fill: string;
};
  ...
  return (
    <div className="rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 text-xs shadow-lg max-w-xs">
      ...
      <p className="mt-1 text-slate-600 dark:text-slate-300">
        Dollar Exposure: {money(row.dollarExposure)}
      </p>
    </div>
  );
}
...
type Props = {
  rows: CFOHeatmapBarRow[];
  /** Total slot height (aligns with gauge panel). */
  slotHeight?: number;
  /** Override bar plot height only. */
  chartHeight?: number;
  onSelect?: (row: CFOHeatmapBarRow) => void;
  showClickHint?: boolean;
};

export default function CFORiskHeatmapVisual({
  rows,
  ...
  const chartData = useMemo((): ChartRow[] => {
    return rows.map((row) => {
      const shortType =
        ...
        cfoHeatmapIssueTypeShortName(row.issueType);
      return {
        ...row,
        shortIssueType: shortType,
        chartLabel: shortLabel(shortType, 36),
        fill: heatmapSeverityHex(row.severity),
      };
    });
  }, [rows]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-slate-400 dark:text-slate-500">Top 8 · bar length = dollar exposure</span>
      </div>
      ...
      <div className="w-full flex-1 min-h-0 shrink-0" style={{ height: plotHeight, minHeight: plotHeight }}>
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            >
              {chartData.map((entry) => (
                <Cell key={entry.alertId} fill={entry.fill} />
              ))}
              <LabelList
                dataKey="dollarExposure"
                position="right"
