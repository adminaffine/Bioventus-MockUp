...
import { CFO_TEAM_OWNERS, type CFOTeamOwner } from "../../config/cfoTeamOwners";
...
type Props = {
  ...
  onClose: () => void;
  onConfirm: (owner: CFOTeamOwner) => Promise<void>;
};

export default function CFOReassignModal({
  open,
  alertId,
  orderId,
  currentAssignee,
  ...
  const selected = CFO_TEAM_OWNERS.find((o) => o.id === selectedId);
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 px-5 py-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Reassign Alert</h2>
          </div>
        ...
        <div className="px-5 py-4 space-y-4 text-sm">
          <p className="text-slate-600 dark:text-slate-300">
            <span className="font-medium text-slate-800 dark:text-slate-100">{alertId}</span> · {orderId}
          </p>
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {CFO_TEAM_OWNERS.map((owner) => (
              <label
                key={owner.id}
        ...
        <div className="flex justify-end gap-2 border-t border-slate-200 dark:border-slate-700 px-5 py-4">
          <button type="button" onClick={onClose} disabled={saving} className="px-4 py-2 text-sm rounded-lg border border-slate-300 dark:border-slate-600">
            Cancel
          </button>
          <button
            type="button"
